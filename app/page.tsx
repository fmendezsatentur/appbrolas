...
export default function MarketplacePage() {
  ...
  }, [search, condition, color, sellerSearch])
  ...
  // Group same card from same seller into one tile, summing quantity
  ...
  const hasFilters = !!(search || condition || color || sellerSearch)
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        <div className="mb-8 space-y-1">
          ...
          <p className="text-muted-foreground">
            {grouped.length} cartas disponibles
          </p>
        </div>

        {/* Filters */}
        <div className="space-y-3 mb-6">
          {/* Row 1: card search + seller search */}
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {COLORS.map((c) => (
              <button
                ...
                type="button"
                title={c.label}
                onClick={() => setColor((prev) => (prev === c.code ? null : c.code))}
                className={`w-7 h-7 rounded-full border-2 flex items-center justify-center text-xs transition-all ${c.bg} ${c.text} ${
                  color === c.code
                    ? `${c.border} ring-2 ring-offset-1 ring-offset-background ring-primary scale-110`
            ...
            {hasFilters && (
              <Button
                ...
                variant="ghost"
                className="gap-1.5"
                onClick={() => { setSearch(''); setCondition(null); setColor(null); setSellerSearch('') }}
              >
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="aspect-[2/3] rounded-xl bg-muted/50 animate-pulse" />
            ))}
          </div>
        ) : grouped.length === 0 ? (
          <div className="text-center py-20 text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              {hasFilters ? 'Probá con otros filtros' : 'Sé el primero en publicar una carta'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
